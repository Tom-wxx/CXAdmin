...
public class SysLoginLogController {
    ...
    public PageResult<SysLoginLog> list(
            PageQuery pageQuery,
            @Parameter(description = "用户账号") @RequestParam(required = false) String username,
            @Parameter(description = "登录IP地址") @RequestParam(required = false) String ipaddr,
    ...
    @GetMapping("/{infoId}")
    public Result<SysLoginLog> getInfo(@Parameter(description = "日志ID") @PathVariable Long infoId) {
    ...
    public Result<Void> remove(@Parameter(description = "日志ID数组") @PathVariable Long[] infoIds) {
